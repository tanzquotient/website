...
class Command(NoArgsCommand):
    style_names = ['Walzer', 'Discofox', 'Foxtrott', 'Quick-Step', 'Jive', 'Rumba', 'Tango', 'Salsa Cubana',
                   u'Salsa Puertorriqueña', 'Bachata', 'Lindy Hop', 'Zouk']
    course_names = ['Discofox', 'Social', 'Tango', 'Salsa Cubana', u'Salsa Puertorriqueña', 'Bachata', 'Lindy Hop',
                    'Zouk']
    room_names = ['CAB Foodlab', 'GEP/Alumni-Pavillon', 'HXE', 'Raum der Stille', u'ASVZ Hönggerberg Arena 3',
                  u'PH, LAC-C014']
    room_desc = [
        'Das foodLAB ist das DSR Restaurant im CAB. Es befindet sich im obersten Stock direkt über dem Haupteingang. Die Treppe hoch, dann gerade aus. Auf der rechten Seite geht eine Metalltreppe in den obersten Stock. Oder nach dem Haupteingang gerade nach hinten laufen, dann links den Lift ins oberste Stockwerk.',
    ...
    room_url = [
        'https://www.google.ch/maps/place/ETHZ+CAB,+8006+Z%C3%BCrich/@47.3775785,8.5486153,17z/data=!3m1!5s0x479aa0a661fe00ab:0x3f31372993b75a8e!4m7!1m4!3m3!1s0x479aa0a62c4b4f21:0xb86c4ed668042726!2sAlumni+Pavillon,+8001+Z%C3%BCrich!3b1!3m1!1s0x479aa0a661c27a25:0x8fc386de863192d0',
    ...
    def handle_noargs(self, **options):
        ...
        user = User.objects.create_user('simon', 'siwehrli@student.ethz.ch', 'a', first_name="Simon",
                                        last_name="Wehrli")
        user.is_staff = True
        ...
        for style_name in self.style_names:
            Style.objects.get_or_create(name=style_name,
                                        defaults={'url_info': "http://de.wikipedia.org/wiki/{}".format(style_name)})
        Style.objects.get_or_create(name="Discofox",
                                    defaults={'url_info': "http://de.wikipedia.org/wiki/{}".format("Discofox")})
        ...
        d.styles = [Style.objects.get(name='Walzer'), Style.objects.get(name='Tango'), Style.objects.get(name='Jive'),
                    Style.objects.get(name='Discofox')]
        d.save()

        for i, room_name in enumerate(self.room_names):
            Room.objects.get_or_create(name=room_name,
                                       defaults={'description': self.room_desc[i], 'url': self.room_url[i]})

        pHS14q1 = \
            Period.objects.get_or_create(date_from=datetime.date(2014, 9, 15), date_to=datetime.date(2014, 10, 31))[0]
        pHS14q2 = \
            Period.objects.get_or_create(date_from=datetime.date(2014, 11, 2), date_to=datetime.date(2014, 12, 19))[0]
        pFS15q1 = Period.objects.get_or_create(date_from=datetime.date(2015, 2, 16), date_to=datetime.date(2015, 4, 2))[
            0]
        pFS15q2 = \
            Period.objects.get_or_create(date_from=datetime.date(2015, 4, 13), date_to=datetime.date(2015, 5, 29))[0]
        pHS15q1 = \
            Period.objects.get_or_create(date_from=datetime.date(2015, 9, 14), date_to=datetime.date(2015, 10, 30))[0]
        pHS15q2 = \
            Period.objects.get_or_create(date_from=datetime.date(2015, 11, 1), date_to=datetime.date(2015, 12, 18))[0]
        pFS16q1 = \
            Period.objects.get_or_create(date_from=datetime.date(2016, 2, 22), date_to=datetime.date(2016, 3, 24))[0]
        pFS16q2 = Period.objects.get_or_create(date_from=datetime.date(2016, 4, 4), date_to=datetime.date(2016, 6, 3))[
            0]
        o = Offering.objects.get_or_create(name="HS 2014 Q1",
                                           defaults={'period': pHS14q1, 'display': True, 'active': True})[0]
        Offering.objects.get_or_create(name="HS 2014 Q2", defaults={'period': pHS14q2, 'display': True, 'active': True})
        Offering.objects.get_or_create(name="FS 2015 Q1",
                                       defaults={'period': pFS15q1, 'display': False, 'active': False})
        Offering.objects.get_or_create(name="FS 2015 Q2",
                                       defaults={'period': pFS15q2, 'display': False, 'active': False})
        Offering.objects.get_or_create(name="HS 2015 Q1",
                                       defaults={'period': pHS15q1, 'display': False, 'active': False})
        Offering.objects.get_or_create(name="HS 2015 Q2",
                                       defaults={'period': pHS15q2, 'display': False, 'active': False})
        Offering.objects.get_or_create(name="FS 2016 Q1",
                                       defaults={'period': pFS16q1, 'display': False, 'active': False})
        Offering.objects.get_or_create(name="FS 2016 Q2",
                                       defaults={'period': pFS16q2, 'display': False, 'active': False})
        ...
        c = Course.objects.get_or_create(name="Lindy Hop 1",
                                         defaults={'type': CourseType.objects.get(name='Lindy Hop 1'), 'offering': o,
                                                   'room': Room.objects.get(name=self.room_names[3])})[0]
