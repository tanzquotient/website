...
def calculate_relevant_experience(user,course):
    ...
    return [s.course for s in mymodels.Subscribe.objects.filter(user=user,confirmed=True,course__type__styles__id__in=relevant_exp).exclude(course=course).order_by('course__type__level').distinct().all()]
