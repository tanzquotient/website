...
class RoomUsageApiView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response({"detail": "room_id must be integer(s)"}, status=HTTP_400_BAD_REQUEST)
        ...
        if len(found_ids) != len(set(room_ids_int)):
            return Response({"detail": "one or more room_id values not found"}, status=HTTP_400_BAD_REQUEST)
        ...
        if end_dt <= start_dt:
            if start_dt.date() == end_dt.date():
                end_dt = timezone.make_aware(
                    datetime.combine(end_dt.date(), time.max), timezone.get_current_timezone()
                )
            else:
        ...
        occurrences = (
            ...
            .filter(
                (Q(room__in=room_ids_int) | (Q(room__isnull=True) & Q(course__room__in=room_ids_int))),
                start__gte=start_dt,
                end__lte=end_dt,
            )
            .order_by("start")
