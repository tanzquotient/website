        ...
        if self.course.type.couple_course:
            ...
            if single_or_couple == SingleCouple.SINGLE:
                ...
                if not cleaned_data.get("lead_follow"):
                    error = ValidationError(message=_('This field is required.'), code='lead follow preference empty')
            ...
            if single_or_couple == SingleCouple.COUPLE:
                ...
                if self.course.max_subscribers is not None:
                    ...
                    if self.course.get_free_places_count() < 2:
                        self.add_error('partner_email', ValidationError(
                            message=_('At least two spots need to be available to sign up as a couple.'),
                    ...
                    if not self.course.has_free_places_for_leaders():
                        self.add_error('partner_email', ValidationError(
                            message=_('You can not sign up with a partner anymore, since one of you needs to be the '
