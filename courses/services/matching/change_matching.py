        ...
        if partner_subs.count() == 1:
            ...
            if (
                ...
                with reversion.create_revision():
                    ...
                    reversion.set_comment("Natching state corrected to couple")

    if corrected_count:
        messages.add_message(
            request,
            messages.SUCCESS,
            _("{} subscriptions ({} couples) corrected successfully").format(
...
def unmatch_partners(subscriptions, request):
    ...
    for s in subscriptions.all():
        if s.state in [models.SubscribeState.NEW, models.SubscribeState.WAITING_LIST]:
            ...
            partner_subs = subscriptions.filter(user=s.partner, course=s.course)
            if (
                partner_subs.count() == 1
                and s.matching_state in allowed_states
                and partner_subs.first().matching_state in allowed_states
            ):
                _unmatch_person(s)
    ...
    if corrected_count:
        messages.add_message(
            request,
            messages.SUCCESS,
            _("{} couples unmatched successfully").format(corrected_count),
        )
    if invalid_state_count:
        messages.add_message(
            request,
            messages.WARNING,
            _("{} subscriptions can not be unmatched because already CONFIRMED").format(
                invalid_state_count
            ),
        )
    if invalid_matching_state_count:
        messages.add_message(
            ...
            messages.WARNING,
            _(
                "{} subscriptions can not be unmatched because invalid matching state"
            ).format(invalid_matching_state_count),
        )
...
def breakup_couple(subscriptions, request):
    ...
    if corrected_count:
        messages.add_message(
            request,
            messages.SUCCESS,
            _("{} couples broken up successfully").format(corrected_count),
        )
    if invalid_state_count:
        messages.add_message(
            request,
            messages.WARNING,
            _("{} couples can not be broken up because already CONFIRMED").format(
                invalid_state_count
            ),
        )
    if invalid_matching_state_count:
        messages.add_message(
            request,
            messages.WARNING,
            _("{} couples can not be broken up because invalid matching state").format(
