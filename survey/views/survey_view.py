...
def _get_survey_instance(url_key: str):
    ...
    return (
        SurveyInstance.objects.filter(url_key=url_key)
        .prefetch_related(
            "survey__questiongroup_set",
            "survey__questiongroup_set__question_set",
            "survey__questiongroup_set__question_set__scale",
        )
        .get()
    )
...
def survey_view(request, survey_id: int, url_key: Optional[str] = None) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if survey_instance.has_answers():
            ...
            return render(
                request, "survey/survey_done.html", {"survey_instance": survey_instance}
            )
