...
from courses.services.cache import invalidate_course_reviews_cache
    ...
    if answer_id:
        ...
        if answer.hide_from_public_reviews != hide:
            with reversion.create_revision():
                ...
                invalidate_course_reviews_cache()

                reversion.set_user(request.user)
                fallback_comment = (
                    "Answer was hidden." if hide else "Answer was made visible again."
                )
...
def changed_answers(request: HttpRequest) -> HttpResponse:
    ...
    for version in (
        Version.objects.get_for_model(Answer)
        .exclude(revision__comment="Initial version.")
        .all()
    ):
        grouped_by_version[version.object].append(
            (version.revision, version.field_dict)
        )

    changed_answers = sorted(
        grouped_by_version.items(),
        key=lambda t: t[0].survey_instance.last_update,
        reverse=True,
    )

    return render(
        request,
        "survey/changed_answers.html",
        context=dict(changed_answers=changed_answers),
    )
