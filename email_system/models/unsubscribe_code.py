...
class UnsubscribeCode(Model):
    user = ForeignKey(
        to=User, related_name="unsubscribe_codes", on_delete=CASCADE, blank=False
    )
    ...
    def get_unsubscribe_url(self, context):
        url = reverse(
            "email_system:unsubscribe",
            kwargs={"context": context, "user_id": self.user.id, "code": self.code},
        )
