...
if __name__ == '__main__':
    ...
    EXCLUDE_DIRS = ['migrations', 'scripts',]
    EXCLUDE_FILES = ['manage.py',]

    # parse the command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('directory', help='the directory to generate the UML diagrams for')
    parser.add_argument('-o', '--output_dir', help='directory to save the generated files in')
    ...
    while i < len(files_to_analyse):
        ...
        while i + 1 < len(files_to_analyse) and files_to_analyse[i+1].startswith(current_dir):
