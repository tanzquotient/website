...
def stacked_bar_chart(labels: list[str], data: list[DataSeries]) -> Figure:
    sum_by_label = {
        label: sum([data_series.values[idx] or 0 for data_series in data])
    ...
    return Figure(
        data=[
            Bar(
                ...
                y=labels,
                text=[
                    f"{value * 100 / sum_by_label[label]:.1f} %"
                    if value and sum_by_label[label] != 0
                    else None
        ...
        layout=Layout(
            ...
            legend=Legend(
                x=0.5, y=1.02, orientation="h", yanchor="bottom", xanchor="center"
            ),
        ),
    )
