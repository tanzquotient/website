...
class SubscriptionPayment(models.Model):
    ...
    payment = models.ForeignKey(
        Payment, related_name="subscription_payments", on_delete=models.PROTECT
    )
    subscription = models.ForeignKey(
        Subscribe, related_name="subscription_payments", on_delete=models.PROTECT
    )
    ...
    def clean(self):
        ...
        if self.amount and (self.amount > self.payment.amount):
            raise ValidationError(
                "The available payment amount is not sufficient to allow the association of {}.".format(
                    self.amount
                )
            )
