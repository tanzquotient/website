...
def offering_finance_teachers_export(request: HttpRequest, offering: int) -> HttpResponse:
    export_format = request.GET.get('format', 'csv')
    from payment import services
    export_name, personal_data, courses = services.offering_finance_teachers(Offering.objects.filter(id=offering).all())
