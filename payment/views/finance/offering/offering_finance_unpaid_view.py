...
class OfferingFinanceUnpaidView(PermissionRequiredMixin, TemplateView, ProcessFormView, FormMixin):
    ...
    def get_context_data(self, **kwargs: dict) -> dict:
        ...
        subscriptions = list(Subscribe.objects.filter(course__offering_id=kwargs['offering'],
                                                            state=SubscribeState.CONFIRMED).select_related(
            'user', 'user__profile', 'course', 'course__offering').prefetch_related('payment_reminders').order_by(
            'user__first_name').all())
        context['subscriptions'] = subscriptions
        context['count'] = len(subscriptions)
        context['users_count'] = len({subscription.user_id for subscription in subscriptions})
        context['courses_count'] = len({subscription.course_id for subscription in subscriptions})
        context['open_total'] = sum([subscription.open_amount() for subscription in subscriptions])
        return context
    ...
    def post(self, request, **kwargs):
        ...
        if "remind" in request.POST:
            ...
            if remind == 'all':
                for subscription in Subscribe.objects.filter(course__offering_id=kwargs['offering'],
        ...
        if "mark_as_paid" in request.POST:
            subscription: Subscribe = Subscribe.objects.get(id=request.POST['mark_as_paid'])
